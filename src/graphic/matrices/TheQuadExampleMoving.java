...
public class TheQuadExampleMoving {
    ...
    private Vector3f cameraRot = null;
    ...
    private void setupTextures() {
        texIds[0] = 0; //this.loadPNGTexture("assets/images/stGrid1.png", GL13.GL_TEXTURE0);
        texIds[1] = 0; //this.loadPNGTexture("assets/images/stGrid2.png", GL13.GL_TEXTURE0);
    ...
    private void setupShaders() {
        ...
        int vsId = this.loadShader("res/shaders/shader.vert", GL20.GL_VERTEX_SHADER);
        // Load the fragment shader
        int fsId = this.loadShader("res/shaders/shader.frag", GL20.GL_FRAGMENT_SHADER);
    ...
    private void logicCycle() {
        ...
        float dt = 0.16f;
        if (Mouse.isGrabbed()) {
            cameraRot.y += mouseSpeed * dt * (float) (Mouse.getDX());
            cameraRot.x -= mouseSpeed * dt * (float) (Mouse.getDY());

            cameraRot.x = Math.min(cameraRot.x, (float) Math.PI / 2);
            cameraRot.x = Math.max(cameraRot.x, -(float) Math.PI / 2);

            cameraRot.y %= 2 * Math.PI;
            cameraRot.x %= 2 * Math.PI;
        }
        ...
        if (flyUp && !flyDown) {
            cameraPos.y -= speedY * delta * 0.003f;
        }
        if (flyDown && !flyUp) {
            cameraPos.y += speedY * delta * 0.003f;
        }
        ...
        // Translate camera
        Matrix4f.rotate(cameraRot.x, new Vector3f(1, 0, 0), viewMatrix, viewMatrix);
        Matrix4f.rotate(cameraRot.y, new Vector3f(0, 1, 0), viewMatrix, viewMatrix);
        Matrix4f.rotate(cameraRot.z, new Vector3f(0, 0, 1), viewMatrix, viewMatrix);
    ...
    public void moveFromLook(float dx, float dy, float dz) {
        this.cameraPos.z += dx * (float) cos(toRadians(cameraRot.y - 90)) + dz * cos(toRadians(cameraRot.y));
        this.cameraPos.x -= dx * (float) sin(toRadians(cameraRot.y - 90)) + dz * sin(toRadians(cameraRot.y));
        this.cameraPos.y += dy * (float) sin(toRadians(cameraRot.x - 90)) + dz * sin(toRadians(cameraRot.x));
    }
